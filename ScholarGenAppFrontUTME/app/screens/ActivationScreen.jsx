    ...
    const initializePayment = async () => {
        ...
        try {
            ...
            if (paystackData && paystackData.authorization_url) {
                ...
                setTransactionRef(paystackData.reference || serverResponse.reference);
                setShowGateway(true);
            } else {
                Toast.show({
                    type: 'error',
    ...
    const verifyPayment = async (reference) => {
        ...
        try {
            ...
            if (data.status === 'success' || data.message === 'Payment successful' || data.user?.premium?.active) {
                ...
                // --- CRITICAL FIX START ---
                // 1. Check if the backend sent a new Token (Authorization Token)
                // Often updated permissions come with a new token. We check multiple likely places.
                const newToken = data.token || data.access_token || (data.user && data.user.token);

                if (newToken) {
                    // Explicitly save to the key "userToken" which VideosScreen uses
                    await AsyncStorage.setItem("userToken", newToken);
                    console.log("ActivationScreen: New Premium Token Saved!");
                }
                // --- CRITICAL FIX END ---
